...
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return tg:IsOnField() and tg:IsSetCard(0x759) end
end
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local at=Duel.GetAttacker()
	if at and at:IsRelateToBattle() and at:IsFaceup() and at:IsSetCard(0x759) then
		--Increase ATK
		local e1=Effect.CreateEffect(c)
...
function s.ctfilter(c)
	return c:IsFaceup() and c:IsSummonLocation(LOCATION_EXTRA) and c:IsSetCard(0x759)
end
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if ct>0 and Duel.IsExistingTarget(s.tdfilter,tp,0,LOCATION_ONFIELD,1,nil) then
